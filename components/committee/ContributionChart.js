...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: 'var(--teal)' }}>${total.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</div>
      <div style={{ color: 'var(--text-dim)' }}>{num_contributions.toLocaleString()} contribution{num_contributions !== 1 ? 's' : ''}</div>
    </div>
  );
}
