...
export async function loadCandidate(acctNum) {
  ...
  const { data: pcLinksRaw } = await db
    ...
    .select('pc_acct_num, pc_name, pc_type, link_type, confidence_tier, is_candidate_specific, signal_evidence')
  ...
  for (const c of pcCommitteeRows || []) _pcCommitteeMap[c.acct_num] = c;

  // ── 5. Expenditure summary + top vendors + news ─────────────────────────────
