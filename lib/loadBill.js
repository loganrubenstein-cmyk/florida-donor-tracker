...
export async function loadBill(slug, yearOverride) {
  ...
  if (slug) {
    // Migration 039 populates bill_slug on both tables with a b-tree index, so
    // join on the canonical slug (hb-1019) rather than the zero-padded
    // bill_number (H1019). This also makes the path prefix-agnostic — HJR,
    // SCR, HM, SM, etc. all resolve through the same slug column.
    ...
    // Pick the session_id whose vote_date year matches activeYear exactly.
    // FL bill numbers reset each session (e.g. HB 1019 in 2024 is a different
    // bill from HB 1019 in 2025) so NEVER fall back to a different session's
    // roll-call data — emit empty sponsors/votes if the requested year has no
    // matching session.
    ...
    const activeSessionId = sessionsByYear[String(activeYear)] ?? null;
