...
/**
 * Average contribution size — total $ divided by count of gifts.
 * Returns null if count is missing or zero (so callers can skip rendering cleanly).
 * avgContribution(123456, 78) → 1582.77
 */
export function avgContribution(total, count) {
...
/**
 * Format an avg contribution amount for display. Under $1k shows cents,
 * otherwise whole dollars. Returns '—' if not computable.
 * fmtAvgContribution(123456, 78) → "$1,583"
 * fmtAvgContribution(412.37, 1)  → "$412.37"
 * fmtAvgContribution(0, 0)       → "—"
 */
export function fmtAvgContribution(total, count) {
  const avg = avgContribution(total, count);
  if (avg === null) return '—';
  return avg < 1000 ? fmtMoney(avg, 2) : fmtMoney(avg, 0);
}
