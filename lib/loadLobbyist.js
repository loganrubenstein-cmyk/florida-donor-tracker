...
export async function loadLobbyist(slug) {
  ...
  const [
    ...
    db.from('bill_disclosures')
      ...
      .ilike('lobbyist', lobbyist.name.split(/\s+/).filter(t => t.length > 1).map(t => t.replace(/\.$/, '')).join('%') + '%')
  ...
  // Aggregate bill filing counts into top 10 — key by (bill_slug, year) because
  // the same bill number in different sessions refers to different legislation.
  const billCounts = {};
  for (const r of billRows || []) {
    if (!r.bill_slug) continue;
    const key = `${r.bill_slug}|${r.year}`;
    if (!billCounts[key]) {
      billCounts[key] = { bill_slug: r.bill_slug, bill_canon: r.bill_canon, year: r.year, filings: 0, principals: new Set() };
    }
    billCounts[key].filings += 1;
    if (r.principal) billCounts[key].principals.add(r.principal);
  }
  const topBills = Object.values(billCounts)
    .sort((a, b) => b.filings - a.filings)
    .slice(0, 10)
    .map(b => ({ bill_slug: b.bill_slug, bill_canon: b.bill_canon, year: b.year, filings: b.filings, num_principals: b.principals.size }));
  ...
  return { ...lobbyist, principals, compHistory, totalComp, topBills, news };
}
...
export async function loadPrincipal(slug) {
  ...
  // Donation matches + quarterly comp + top firms + news (parallel)
  const [{ data: matchRows }, { data: compRows }, { data: firmRows }, { data: issueRows }, { data: newsRows }] = await Promise.all([
    ...
    db.from('lobbying_firm_clients')
      ...
      .limit(10),
    db.from('lobby_principal_issues')
      .select('issue, disclosures, bills, lobbyists, earliest_year, latest_year')
      .eq('principal', principal.name)
      .order('disclosures', { ascending: false })
      .limit(15),
    db.from('news_entity_articles')
      .select('article_title, article_url, article_outlet, article_published, article_snippet, source')
      .eq('entity_type', 'principal')
      .eq('entity_slug', slug)
      .order('article_published', { ascending: false })
      .limit(8),
  ]);
  ...
  const issueAreas = (issueRows || []).map(r => ({
    issue:        r.issue,
    disclosures:  r.disclosures || 0,
    bills:        r.bills || 0,
    lobbyists:    r.lobbyists || 0,
