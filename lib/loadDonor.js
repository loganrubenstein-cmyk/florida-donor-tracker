...
export async function loadDonor(slug) {
  ...
  // Catches cases where the donor slug and principal slug diverge on common
  // prefixes/suffixes: "THE GEO GROUP, INC." → donor `the-geo-group-inc` vs
  // principal `geo-group-inc`. Strips THE/A/AN + corporate suffixes for a
  // looser ilike match on the name column.
  if (lobbyistPrincipals.length === 0 && donor.name) {
    const stripped = donor.name
      .toUpperCase()
      .replace(/^(THE|A|AN)\s+/, '')
      .replace(/,?\s+(INC|INCORPORATED|LLC|L\.L\.C\.|CORP|CORPORATION|CO|COMPANY|LTD|LP)\.?$/g, '')
      .trim();
    if (stripped.length >= 4) {
      const { data: nameMatchRows } = await db
        ...
        .ilike('name', `%${stripped}%`)
        .limit(5);
      const pick = (nameMatchRows || []).find(r => {
