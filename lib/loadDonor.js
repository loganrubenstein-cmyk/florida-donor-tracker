...
const CORP_SUFFIXES = new Set([
  ...
  'CORP', 'CORPORATION', 'CO', 'COMPANY', 'COS',
  'PA', 'PLLC', 'PC', 'PL',
  'NA', 'USA',
]);

function normalizeCorpName(raw) {
  if (!raw) return '';
  let s = String(raw).toUpperCase().replace(/\./g, '');
  s = s.split(',').map(x => x.trim()).filter(x => x && !CORP_SUFFIXES.has(x)).join(' ');
  const words = s.split(/\s+/).filter(Boolean);
  while (words.length && CORP_SUFFIXES.has(words[0])) words.shift();
...
export async function loadDonor(slug) {
  ...
  // ── 3. Top candidates ───────────────────────────────────────────────────────
  // Office/party/year come from the donor_candidates view directly now
  // (migration 046 denormalized them from the candidates table); the previous
  // `candidates(...)` embed returned PGRST200 silently on an aggregating view.
  const { data: candidateRows } = await db
    .from('donor_candidates')
    .select('acct_num, candidate_name, total, num_contributions, party_code, office_desc, election_year')
    .eq('donor_slug', decodedSlug)
    .order('total', { ascending: false })
    .limit(25);
  ...
  const candidates = (candidateRows || []).map(r => ({
    ...
    office:          r.office_desc    || null,
    party:           r.party_code     || null,
  ...
  // has_lobbyist_link from donors_mv is just a strict slug-join EXISTS check
  // against `principals`, which misses most real name matches. Always query
  // `principal_donation_matches` directly — the upper/lower(contributor_name)
  // indexes (migration 042) make the equality-style ilike a B-tree lookup.
  ...
  // Fallback A: direct slug match against principals table
  ...
  // Fallback B: fuzzy name match against principals table.
  // Handles form variations:
  //   "THE GEO GROUP, INC."        ↔ "Geo Group, Inc, The"
  //   "FLORIDA POWER & LIGHT CO."  ↔ "Florida Power & Light Company"
  //   "GUNSTER, YOAKLEY & STEWART, P.A." ↔ "Gunster Yoakley & Stewart"
  // by normalizing both sides: upper-case, strip periods, drop any
  // comma-segment that is entirely a suffix token (INC/LLC/THE/...),
  // then strip suffix words from head and tail.
  if (lobbyistPrincipals.length === 0 && donor.name) {
    const stripped = normalizeCorpName(donor.name);
    if (stripped.length >= 4) {
      const firstCore = stripped.split(' ')[0];
      const { data: nameMatchRows } = await db
        .from('principals')
        .select('slug, name')
        .ilike('name', `%${firstCore}%`)
        .limit(50);
      const candidates = (nameMatchRows || []).map(r => ({
        ...r,
        _norm: normalizeCorpName(r.name),
      }));
      const pick =
        candidates.find(r => r._norm === stripped) ||
        candidates.find(r => r._norm.includes(stripped) || stripped.includes(r._norm));
      if (pick) {
        lobbyistPrincipals = [{
          principal_name: pick.name,
          principal_slug: pick.slug,
          match_score: 0.85,
        }];
      }
    }
