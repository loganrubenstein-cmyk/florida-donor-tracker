...
**Update 2026-04-23:** User surfaced that `Daily — Contributions Scrape` has been failing most days. Root cause from Apr 22 run `24777169912`: `psycopg2.OperationalError: could not receive data from server: Connection timed out` at `41_load_contributions.py:449` (the final `SELECT COUNT(*) FROM contributions WHERE recipient_type='committee'`). The load chunks ran ~2h14m; pgbouncer / the underlying Postgres silently dropped the socket, then the terminal count query hit the dead connection and the script crashed after uploading artifacts (3h58m total, just inside the 240-min workflow timeout). Fix applied: added TCP keepalives (`keepalives=1, idle=30, interval=10, count=5`) to `psycopg2.connect(...)` in scripts 41, 42, 43, 44 — all four heavy LOAD steps that run in the daily workflows. Keeps the socket warm during the multi-hour chunked COPY so trailing queries don't find a zombie connection.
