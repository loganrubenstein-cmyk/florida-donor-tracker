...
def build_transfer_stats(
    ...
    """
    Compute per-committee transfer stats from transfers.csv.

    Returns {acct_num: {total_transferred_out, total_transferred_in,
                        top_transfer_recipients}}
    Matches transferor/transferee names against committee registry by
    upper-cased name (exact match after strip).
    """
    name_to_acct = {}
    ...
    tdf["transferor_acct"] = tdf["transferor_name"].str.strip().str.upper().map(name_to_acct)
    tdf["transferee_acct"] = tdf["transferee_name"].str.strip().str.upper().map(name_to_acct)

    stats: dict = {}

    for acct, group in tdf[tdf["transferor_acct"].notna()].groupby("transferor_acct"):
        top_recipients = (
            group.groupby("transferee_name")["amount"]
            .sum()
        ...
        if acct not in stats:
            stats[acct] = {}
        stats[acct]["total_transferred_out"] = round(float(group["amount"].sum()), 2)
        stats[acct]["top_transfer_recipients"] = [
            {"name": name, "total_amount": round(float(amt), 2)}
            for name, amt in top_recipients.items()
        ]

    for acct, group in tdf[tdf["transferee_acct"].notna()].groupby("transferee_acct"):
        if acct not in stats:
            stats[acct] = {}
        stats[acct]["total_transferred_in"] = round(float(group["amount"].sum()), 2)
...
def build_per_committee_files(
    ...
    Returns {acct_num: {acct_num, committee_name, total_received,
                        ...
                        total_transferred_out, total_transferred_in,
                        top_transfer_recipients}}
    """
    work = df.copy()
    work["committee_acct"] = work["source_file"].apply(derive_committee_acct)
    ...
    for acct, group in work.groupby("committee_acct"):
        ...
        results[acct] = {
            ...
            "total_transferred_out": ts.get("total_transferred_out", 0.0),
            "total_transferred_in":  ts.get("total_transferred_in",  0.0),
            "top_transfer_recipients": ts.get("top_transfer_recipients", []),
        }
    return results
...
def main(force: bool = False) -> int:
    ...
    # Load transfers (optional — only available after scripts 11 + 12)
    transfers_path = PROCESSED_DIR / "transfers.csv"
    transfer_stats = None
    if transfers_path.exists():
        print("Loading transfers.csv ...", flush=True)
        tdf = pd.read_csv(transfers_path, dtype=str, low_memory=False)
        tdf["amount"] = pd.to_numeric(tdf["amount"], errors="coerce").fillna(0.0)
        if "transferor_name" in tdf.columns and "transferee_name" in tdf.columns:
            transfer_stats = build_transfer_stats(tdf, committees_df)
            print(f"  Transfer stats computed for {len(transfer_stats):,} committees\n")
        else:
            print("  transfers.csv missing expected columns — skipping transfer stats\n")
    else:
        print("transfers.csv not found — transfer fields will be zeroed (run scripts 11+12 to add)\n")
    ...
    per_committee = build_per_committee_files(df, committees_df, transfer_stats=transfer_stats)
    ...
    meta = {
        ...
        "total_committees_with_transfers": len(transfer_stats) if transfer_stats else 0,
