...
def main(force: bool = False) -> int:
    ...
    # Augment with fund transfer edges (if transfers.csv exists)
    transfers_path = PROCESSED_DIR / "transfers.csv"
    if transfers_path.exists():
        print("Loading transfers.csv for transfer edges ...", flush=True)
        ...
        if "transferor_name" in tdf.columns and "transferee_name" in tdf.columns:
            ...
            transfer_pairs = (
                tdf.groupby(["transferor_name", "transferee_name"])["amount"]
                .agg(total_amount="sum", num_transfers="count")
                .reset_index()
            )

            for _, row in transfer_pairs.iterrows():
                transferor = str(row["transferor_name"]).strip()
                transferee = str(row["transferee_name"]).strip()

                transferor_acct = name_lookup.get(_clean_name(transferor))
                transferee_acct = name_lookup.get(_clean_name(transferee))

                source_id = make_node_id(acct_num=transferor_acct, canonical_name=transferor)
                target_id = make_node_id(acct_num=transferee_acct, canonical_name=transferee)

                for node_id, name, acct in [
                    (source_id, transferor, transferor_acct),
                    (target_id, transferee, transferee_acct),
                ]:
                    if node_id not in nodes_dict:
                        nodes_dict[node_id] = {
                            "id": node_id,
                            "label": name,
                            "type": classify_node_type(name, acct),
                            "acct_num": acct,
                            "total_received": 0.0,
                            "total_given": 0.0,
                            "num_contributions_in": 0,
                            "depth": depth_map.get(acct, 99) if acct else 99,
                            "data_pending": acct in pending_accts if acct else False,
                        }
                ...
                edges.append({
                    ...
                    "target": target_id,
                    "total_amount": round(float(row["total_amount"]), 2),
                    "num_contributions": int(row["num_transfers"]),
                    "type": "transfer",
                })

            nodes = list(nodes_dict.values())
            print(f"  Transfer edges added. Total edges: {len(edges):,}, total nodes: {len(nodes):,}")
        else:
            print("  transfers.csv missing expected columns — skipping transfer edges")
    else:
        print("transfers.csv not found — skipping transfer edges (run scripts 11+12 first)")
